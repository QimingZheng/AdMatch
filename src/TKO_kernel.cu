...
BYPASS_HEAD:
        ...
        for (int byt = 0; byt < input_bytes; byt++) {
                ...
                if ((byt < input_bytes - 1 && (is_word_char_tko(input[byt]) ^ is_word_char_tko(input[byt + 1])) == 0) ||
                    (byt == input_bytes - 1 && !is_word_char_tko(input[input_bytes - 1])))
                        continue;
