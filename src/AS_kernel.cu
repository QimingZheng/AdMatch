...
BYPASS_HEAD:
        ...
        for (int byt = 0; byt < input_bytes; byt++) {
                ...
                if ((byt < input_bytes - 1 && (is_word_char_as(input[byt]) ^ is_word_char_as(input[byt + 1])) == 0) ||
                    (byt == input_bytes - 1 && !is_word_char_as(input[input_bytes - 1])))
                        continue;
