...
__global__ void nfa_kernel(unsigned char *input, int *input_offset,
                           Transition *transition_list, int *transition_offset,
...
void run_iNFA(struct ita_scratch &scratch, unsigned char **h_input_array,
    ...
    nfa_kernel<<<array_size, threads_per_block, shem>>>(
        d_input, d_input_offset, scratch.d_transition_list,
        scratch.d_transition_offset, scratch.d_init_st_vec,
        scratch.d_persis_st_vec, d_final_st_vec, vec_len);
