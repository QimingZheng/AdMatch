...
bool StateVector::alloc(unsigned int c) {
    ...
    block_count =
        (state_count + bit_sizeof(ST_BLOCK) - 1) / bit_sizeof(ST_BLOCK);
...
string StateVector::toString() {
    ...
    for (int i = 0; i < byte_count; i++) {
        ...
        for (unsigned char mask = 0x01; mask != 0x00 && states < state_count;
             mask <<= 1) {
            if (int(ptr[i] & mask) != 0) {
                ss << 1;
                active_states++;
...
void get_active_states(ST_BLOCK *st_vec, int block_len,
                       vector<ST_T> &active_states) {
    // For each block
